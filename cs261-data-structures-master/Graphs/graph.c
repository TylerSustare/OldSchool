...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct Vertex *curVert;

	//Create our stack
	struct cirListDeque *stack = malloc(sizeof(struct cirListDeque));
	initCirListDeque(stack);

	//Create set of reachable
	struct cirListDeque *reach = malloc(sizeof(struct cirListDeque));
	initCirListDeque(reach);
	...
	{
		//Have curVert point to the top of the stack
		curVert = backCirListDeque(stack);
		removeBackCirListDeque(stack); //Pop off the stack

		curVert->isVisited = 1; //Mark as visited

		//If our curVert is our destination then return true
		if(curVert == destination)
		{
			removeAllCirListDeque(stack);
			removeAllCirListDeque(reach);
			free(stack);
			free(reach);
			return 1;
		}

		//Otherwise add all neighbors to the stack
		for(i = 0; i < curVert->numNeighbors; i++)
		{
			if(!curVert->neighbors[i]->isVisited)
			{
				addBackCirListDeque(stack, curVert->neighbors[i]);
			}
		}
	...
	//If stack becomes empty and we haven't reached our destination
	//then return false
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while(!isEmptyCirListDeque(stack))
	{
		...
		{
			if(!curVert->neighbors[i]->isVisited)
				addFrontCirListDeque(stack, curVert->neighbors[i]);
		}
