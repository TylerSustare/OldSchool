...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	if(rightIdx < max)
	{
		minIdx = _smallerIndexHeap(heap, leftIdx, rightIdx,compare);

		if(compare(getDynArr(heap, minIdx), getDynArr(heap, pos)) == -1)
		{
			swapDynArr(heap, pos, minIdx);
			_adjustHeap(heap, max, minIdx,compare);
		} else {
			return;
		}
	} else if(leftIdx < max) {
...
void removeMinHeap(DynArr *heap, comparator compare)
{
	...
	_adjustHeap(heap, lastIdx, 0,compare);
}
...
void _buildHeap(DynArr *heap, comparator compare)
{
	...
	while(last > -1)
	{
		_adjustHeap(heap, max, last,compare);
		last--;
	}}
/*
