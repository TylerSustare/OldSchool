...
/*
 ...
 param:  cur	the curent root node
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (1){
        ...
        else if (cur->left == NULL && cur->right == NULL){ // node is a leaf
            return 0;
        }
        else if (val < cur->val){
            cur = cur->left;
 ...
 */
/*----------------------------------------------------------------------------*/
void _removeRigthMost(struct Node *cur) {
    assert(cur != 0);
    if(cur->right == NULL) {
